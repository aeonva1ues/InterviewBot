...
try:
    with open('database/db_properties.txt', 'r', encoding='utf-8') \
            as properties:
        content = properties.read()
except FileNotFoundError:
    # Если файл с свойствами был не найден,
    # то продолжить с значениями по умолчанию
    content = ''
...
if content:
    ...
    if len(arg_lines) >= 5:
        ...
        print(
            ('Вероятно, содержимое файла db_properties.txt было '
                'изменено и было удалено обязательное поле\n '
                'Код будет выполнен с настройками по умолчанию.')
        )


def connect_db(sql_query_func):
    # Функция-декоратор. Осуществляет подключение
    # к базе данных с последующим исполнением запроса из
...
def create_table(table_name: str):
    ...
    assert __name__ == '__main__', \
         (
            'Создание таблицы возможно только с '
            'помощью консольного запуска с аргументом')
...
if __name__ == '__main__':
    # Если запущен через консоль с аргументом --newtable,
    # то создаст новую таблицу
    ...
    if args.newtable:
        ...
        for i in range(len(content)):
            if content[i].startswith('active_table'):
                ...
                with open(
                    'database/db_properties.txt', 'w', encoding='utf-8') \
                        as properties:
                    properties.writelines(content)
    else:
        print(
            'Используйте: "python psqlExecutor.py --tablename NAME" '
