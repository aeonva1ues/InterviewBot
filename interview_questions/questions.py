# Каждая строка, используемая для отправки ботом имеет в начале УНИКАЛЬНЫЙ тэг в формате [TAG]::::.
from . import interview_txt_path


# Функция get_question возвращает строку по заданному тэгу.
def get_question(tag: str) -> str:
    assert tag[0].strip() == '[' and tag[-1].strip() == ']', 'Тэг должен быть заключен в скобки'
